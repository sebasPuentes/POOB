...
public class FightsPanel extends JPanel {
    ...
    private boolean isPlayer1Turn = true; // true = turno del jugador 1, false = turno del jugador 2
    // Elementos gráficos de batalla
    ...
    private JLabel turnLabel; // Etiqueta para mostrar de quién es el turno
    private JLabel messageLabel; // Etiqueta para mostrar mensajes durante el combate
    ...
    // Constantes para posicionamiento de elementos
    ...
    // Sistema de gestión de paneles para acciones
    ...
    private void initializeComponents() {
        ...
        atackButton = createOptionButton("ATACAR");
        ...
        // Agregar botones a la lista para gestión de estilo
        buttons.add(atackButton);
    ...
    private void initializeActionPanels() {
        ...
        mainOptionsPanel.add(atackButton);
    ...
    private void initializeTimer() {
        ...
        turnTimer = new Timer(1000, e -> {
            ...
            if (turnTimeRemaining <= 0) {
                stopTimer();
                try {
                    mainGUI.pooBkemon.afterAction(); // Avanzar turno automáticamente
                    updateTurnInfo();
    ...
    private void addComponents() {
        // Verificar que cada componente no sea nulo antes de añadirlo
        try {
            if (turnLabel != null) {
                add(turnLabel);
            } else {
                System.err.println("Error: turnLabel es nulo");
            }
            if (messageLabel != null) {
                add(messageLabel);
            } else {
                System.err.println("Error: messageLabel es nulo");
            }
            if (timerLabel != null) {
                add(timerLabel);
            } else {
                System.err.println("Error: timerLabel es nulo");
            }
            if (pauseButton != null) {
                add(pauseButton);
            } else {
                System.err.println("Error: pauseButton es nulo");
            }
            if (actionPanel != null) {
                add(actionPanel);
            } else {
                System.err.println("Error: actionPanel es nulo");
            }
        } catch (Exception e) {
            System.err.println("Error al añadir componentes: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void prepareActions() {
        atackButton.addActionListener(e -> {
            showMovesPanel();
            startTimer();
        });
    ...
    private void loadPlayerPokemonImage(String pokemonName) {
        try {
            String[] possiblePaths = {
                "resources/pokedex/" + pokemonName + "Back.png",
                "resources/battle/back/" + pokemonName + ".PNG",
                "resources/pokemon/" + pokemonName + "Back.png"
            };
    ...
    private void loadOpponentPokemonImage(String pokemonName) {
        try {
            String[] possiblePaths = {
                "resources/pokedex/" + pokemonName + "Front.png",
                "resources/battle/frente/" + pokemonName + ".PNG",
                "resources/pokemon/" + pokemonName + "Front.png"
            };
            boolean loaded = false;
            for (String path : possiblePaths) {
                File file = new File(path);
    ...
    protected void showMessage(String message) {
        ...
        // Timer para limpiar el mensaje después de un tiempo
